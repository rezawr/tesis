...
def validation(stop_event, valid_event, thread_num):
    ...
    while not stop_event.is_set():
        ...
        try :
            y_pred_prob = textclassifier.predict(data_reduced)
            y_pred_prob = y_pred_prob.flatten()

            nb_pred = (y_pred_prob > 0.5).astype(int)
...
if __name__ == "__main__":
    ...
    pd.DataFrame(result).to_csv('resultnn/performance.csv', index=False)
    pd.DataFrame(performance).to_csv('resultnn/resource.csv', index=False)
    pd.DataFrame(total_thread).to_csv('resultnn/total_thread.csv', index=False)
    print("Done!")
    print(result)
    print(performance)