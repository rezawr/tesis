...
def train(stop_event):
    ...
    while not stop_event.is_set():
        try :
            ...
            tmp_classifier = LogisticRegression(random_state=42)
...
if __name__ == "__main__":
    ...
    pd.DataFrame(result).to_csv('resultlr/performance.csv', index=False)
    pd.DataFrame(performance).to_csv('resultlr/resource.csv', index=False)
    pd.DataFrame(total_thread).to_csv('resultlr/total_thread.csv', index=False)
    print("Done!")
    print(result)
    print(performance)