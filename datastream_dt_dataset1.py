...
def train(stop_event):
    ...
    while not stop_event.is_set():
        try :
            ...
            tmp_classifier = DecisionTreeClassifier(criterion='entropy', min_samples_split=2, random_state=42)
...
if __name__ == "__main__":
    ...
    pd.DataFrame(result).to_csv('resultdt/performance.csv', index=False)
    pd.DataFrame(performance).to_csv('resultdt/resource.csv', index=False)
    pd.DataFrame(total_thread).to_csv('resultdt/total_thread.csv', index=False)
    print("Done!")
    print(result)
    print(performance)