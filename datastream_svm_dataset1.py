...
def train(stop_event):
    ...
    while not stop_event.is_set():
        try :
            ...
            tmp_classifier = SVC(kernel='linear')
...
if __name__ == "__main__":
    ...
    pd.DataFrame(result).to_csv('resultsvm/performance.csv', index=False)
    pd.DataFrame(performance).to_csv('resultsvm/resource.csv', index=False)
    pd.DataFrame(total_thread).to_csv('resultsvm/total_thread.csv', index=False)
    print("Done!")
    print(result)
    print(performance)